...
void MenuManager::renderGameOver(const Game &game) const {
  ...
  glDisable(GL_LIGHTING);
    ...
    std::string pressEscString = "Press ESC to exit";
    Point2 pressEscStringPosition = {(game.screenSize.X() / 2 - 120), (game.screenSize.Y() / 2 - 100)};

    game.textRenderer.render(Colors::White(), Colors::Black(), scoreString.c_str(), scoreStringPosition, blended, big,
                             true);
    game.textRenderer.render(Colors::White(), Colors::Black(), gameOverString.c_str(), gameOverStringPosition, blended,
                             big, false);
    game.textRenderer.render(Colors::White(), Colors::Black(), pressEscString.c_str(), pressEscStringPosition, blended,
                             small, false);
